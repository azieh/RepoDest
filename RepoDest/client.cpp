...
if (Check(res,"Multiread Vars"))
{
   ...
   qWarning("Dump MB0..MB15 - Var Result : %d\n",Items[0].Result);
   if (Items[0].Result==0)
       hexdump(&MB,16);
   qWarning("Dump EB0..EB15 - Var Result : %d\n",Items[1].Result);
   if (Items[1].Result==0)
       hexdump(&EB,16);
   qWarning("Dump AB0..AB15 - Var Result : %d\n",Items[2].Result);
   if (Items[2].Result==0)
       hexdump(&AB,16);
   qWarning("Dump T0..T7 - Var Result : %d\n",Items[3].Result);
   if (Items[3].Result==0)
       hexdump(&TM,16);         // 8 Timers -> 16 bytes
   qWarning("Dump Z0..Z7 - Var Result : %d\n",Items[4].Result);
   if (Items[4].Result==0)
